...
export const selectOrderById = (number: number) => (state: RootState) => {
  ...
  if (state.orderData.orderList.length || state.ordershistory.orders.length) {
    return (
      //селектор пытается найти заказ с указанным номером в state.feeddata.orders. Если такой заказ найден - он возвращается
      state.orderData.orderList.find((order) => order.number === number) ||
  ...
  if (state.orderData.selectedOrder) {
    return state.orderData.selectedOrder.number === number
